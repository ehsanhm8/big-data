...
### Removing counties that only appear in one year.

```{r}
exclusive_counties <- setdiff(noaa_2018$county, noaa_2023$county)
length(exclusive_counties)
print(exclusive_counties)
```

```{r}

noaa_2018_bothyearcounties <- noaa_2018 |>
  filter(county != exclusive_counties)
...
ggplot(mapping = aes(
  x = noaa_2018$total_events,
  y = noaa_2023$total_events
)) +
  geom_point()
```